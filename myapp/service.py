...
def check_if_registered_doc(doc_id):
    ...
    FUNCTION TO CHECK WHETHER DOCTOR EXISTS IN LOCAL DATABASE
...
def register_doc(request,doc_id):
    ...
    FUNCTION TO REGISTER/ SAVE DOCTOR IN LOCAL DATABASE
...
def login_as_doc(request,doc_id):
    ...
    FUNCTION TO LOGIN AS DOCTOR [WILL CALL check_if_registered_doc()]
...
def getDuration(now,then):
    ...
    FUNCTION TO CALCULATE DIFFERENCE BETWEEN TWO TIME STRINGS
...
def patch_to_url(url, access_token, params={}):
    ...
    GENERIC FUNCTION TO PATCH DATA TO API
...
def patch_appointment(request):
	...
	ADDITIONAL FUNCTION TO PATCH APPOINTMENT DATA TO DRCHRONO API
...
def patch_patient(request):
    ...
    ADDITIONAL FUNCTION TO PATCH PATIENT DATA TO DRCHRONO API
